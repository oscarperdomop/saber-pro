...
const EditarUsuarioModal = ({
  ...
  const { data: programas = [], isLoading: isProgramasLoading } = useQuery<
    Programa[]
  >({
    queryKey: ["programas"],
  ...
  useEffect(() => {
    ...
    setFormValues({
      ...
      semestre_actual:
        usuario.semestre_actual !== undefined &&
        usuario.semestre_actual !== null
  ...
  const editarUsuarioMutation = useMutation<
    ...
    {
      id: string | number;
      data: Partial<Usuario> & { documento: string; password?: string };
    }
  >({
    ...
    onError: (error) => {
      ...
      setErrorMessage(
        backendMessage ?? "No fue posible actualizar el usuario.",
      );
    },
  });

  const handleChange = (
    field: Exclude<keyof FormValues, "is_staff">,
    value: string,
  ) => {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const normalizedCurrent = {
      ...
      correo_institucional: formValues.correo_institucional
        .trim()
        .toLowerCase(),
    ...
    const hasChanges =
      ...
      normalizedCurrent.correo_institucional !==
        normalizedOriginal.correo_institucional ||
      normalizedCurrent.tipo_documento !== normalizedOriginal.tipo_documento ||
      normalizedCurrent.numero_documento !==
        normalizedOriginal.numero_documento ||
      ...
      normalizedCurrent.semestre_actual !==
        normalizedOriginal.semestre_actual ||
    ...
    if (!/^\d{1,10}$/.test(normalizedCurrent.numero_documento)) {
      setErrorMessage(
        "El número de documento debe contener solo números y máximo 10 dígitos.",
      );
      return;
    }
    ...
    const payload: Partial<Usuario> & { documento: string; password?: string } =
      {
        ...
        programa_id: normalizedCurrent.programa_id
          ? Number(normalizedCurrent.programa_id)
          : null,
        genero: normalizedCurrent.genero || null,
        semestre_actual:
          normalizedCurrent.rol === "ESTUDIANTE" &&
          normalizedCurrent.semestre_actual
    ...
    if (normalizedCurrent.rol === "ESTUDIANTE") {
      ...
      if (
        ...
        setErrorMessage(
          "Para estudiantes, el semestre actual es obligatorio y debe estar entre 1 y 10.",
        );
        return;
      }
    }
        ...
        <form noValidate onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label className="col-span-1 sm:col-span-2">
              ...
              <input
                ...
                onChange={(event) =>
                  handleChange("correo_institucional", event.target.value)
                }
                className="w-full rounded-xl border border-usco-ocre/80 px-3 py-2 text-sm text-usco-gris outline-none transition focus:border-usco-vino focus:ring-2 focus:ring-usco-vino/15"
                required
              />
                  ...
                  setFormValues((current) => ({
                    ...
                    is_staff:
                      nextRole === "PROFESOR"
                        ...
                        : nextRole === "ADMIN",
                    semestre_actual:
                      nextRole === "ESTUDIANTE" ? current.semestre_actual : "",
                  }));
                }}
                className="w-full rounded-xl border border-usco-ocre/80 px-3 py-2 text-sm text-usco-gris outline-none transition focus:border-usco-vino focus:ring-2 focus:ring-usco-vino/15"
                required
            ...
            {formValues.rol === "PROFESOR" && (
              <label className="col-span-1 sm:col-span-2 flex items-center gap-2">
                ...
                <span className="text-sm text-gray-900">
                  Otorgar acceso administrativo (Staff)
                </span>
              </label>
            )}
            ...
            <label className="col-span-1">
              ...
              <select
                ...
                onChange={(event) =>
                  handleChange("programa_id", event.target.value)
                }
                className="w-full rounded-xl border border-usco-ocre/80 px-3 py-2 text-sm text-usco-gris outline-none transition focus:border-usco-vino focus:ring-2 focus:ring-usco-vino/15"
            ...
            {formValues.rol === "ESTUDIANTE" && (
              <label className="col-span-1">
                ...
                <input
                  ...
                  onChange={(event) =>
                    handleChange("semestre_actual", event.target.value)
                  }
                  className="w-full rounded-xl border border-usco-ocre/80 px-3 py-2 text-sm text-usco-gris outline-none transition focus:border-usco-vino focus:ring-2 focus:ring-usco-vino/15"
          ...
          <div className="mt-1 border-t border-usco-ocre/60 pt-4">
            ...
            <input
              ...
              onChange={(event) =>
                handleChange("nueva_password", event.target.value)
              }
            ...
            <p className="mt-1 text-xs text-usco-gris/80">
              Si escribes una clave aquí, reemplazará la actual del
              estudiante.
            </p>
          </div>
