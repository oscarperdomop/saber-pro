...
const CargaMasivaPreguntasModal = ({
  ...
  const bulkUploadMutation = useMutation<
    ...
    { file: File; moduloId?: number; categoriaId?: number; competenciaId?: number }
  >({
    ...
    onSuccess: (response) => {
      ...
      const omitidas = response.filas_omitidas ?? 0
      const extra =
        errores > 0 ? ` Se omitieron ${errores} fila(s) con error.` : ''
      const extraOmitidas =
        omitidas > 0 ? ` Se ignoraron ${omitidas} fila(s) vacia(s).` : ''
      const extraLote = response.lote_id ? ` Lote: ${response.lote_id}.` : ''
      setSuccessMessage(
        `Carga completada: ${response.preguntas_creadas} pregunta(s) creadas.${extra}${extraOmitidas} Filas autocompletadas por IA: ${response.filas_con_ia}.${extraLote}`,
      )
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-2xl rounded-2xl border border-usco-ocre/80 bg-white p-6 shadow-xl">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-usco-gris">
                Si una fila trae solo ENUNCIADO y OPCION_CORRECTA, el sistema completara
                distractores faltantes con IA.
              </p>
              <p className="mt-1 text-xs text-usco-gris/80">
                Modulo, categoria y competencia se leen por fila desde el Excel. Estos selectores
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <label className="block">
            <span className="mb-1.5 block text-sm font-semibold text-usco-gris">Modulo</span>
            <select
              value={moduloId}
              onChange={(event) =>
          ...
          <label className="block">
            <span className="mb-1.5 block text-sm font-semibold text-usco-gris">
              Categoria (sobrescribir lote, opcional)
            </span>
            <select
              value={categoriaId}
              onChange={(event) =>
        ...
        {successMessage && (
          <div className="mt-3 rounded-lg border border-green-300 bg-green-50 px-3 py-2 text-sm text-green-700">
            {successMessage}
          </div>
        )}
